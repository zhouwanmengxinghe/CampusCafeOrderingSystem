...
namespace CampusCafeOrderingSystem.Controllers
{
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Add(int id)
        {
            ...
            if (product != null)
            {
                // Enforce single-vendor cart: prevent adding items from different vendors
                if (cart.Any())
                {
                    var existingVendor = cart.First().Product?.VendorEmail ?? string.Empty;
                    var newVendor = product.VendorEmail ?? string.Empty;

                    if (!string.IsNullOrWhiteSpace(existingVendor) &&
                        !string.IsNullOrWhiteSpace(newVendor) &&
                        !string.Equals(existingVendor, newVendor, StringComparison.OrdinalIgnoreCase))
                    {
                        TempData["ErrorMessage"] = "You can only order from one vendor at a time. Please clear your cart before adding items from another vendor.";
